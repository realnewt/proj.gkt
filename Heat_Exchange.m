...
T_FLOW_in=936;      %[K] Initial temperature of mixture
T_FLOW_ut_final=327;      %[K] Temperature after heat final exchange
T_MEDIUM_in=273.15+14
...
if T_FLOW_in>T_MEDIUM_in       %Where T_medium_in is the inlet temperature of the cooling/heating flow 

%Hot to cold    
while T_FLOW_in>T_FLOW_ut_final && k<A_num_max        %While loop that counts number or heat exchange units
k=k+1;
    while j>T_FLOW_ut_final       %While loop that iterates TC_in
    ...
    i=0;
        ...
        T_FLOW_ut=j;        %Outlet temperature
        T_FLOW_medel=(T_FLOW_in+T_FLOW_ut)/2;       

        CP_matrix=[Cp_new(T_FLOW_medel,1,1,1,1,1);
                   Cp_new(T_FLOW_medel,2,2,2,2,2);
                   Cp_new(T_FLOW_medel,3,3,3,3,3);
                   Cp_new(T_FLOW_medel,4,4,4,4,4)];     %[J/Kg*K] Cp each component
     
        %Temperature calculation coolant
        T_MEDIUM_in=273+14;       %Inlet temperature of coolant  
        T_MEDIUM_ut=T_MEDIUM_in+C*(T_FLOW_in-T_FLOW_ut);        %Outlet temperature of coolant     

        %Energy transfer calculations
        q_matrix=F_mass(:,1).*CP_matrix(:,1).*(T_FLOW_in-T_FLOW_ut);      %Energy transfer matrix
        ...
        %Epsilon calculation
        Epsilon=q/(C_min*(T_FLOW_in-T_MEDIUM_in));        %0 < Epsilon < 1
...
T_MEDIUM_ut=T_MEDIUM_in+(T_FLOW_in-T_FLOW_ut)./C;
